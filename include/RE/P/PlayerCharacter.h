...
namespace RE
{
	...
	public:
		...
		void MoveToCell(const NiPoint3& a_position, const NiPoint3& a_rotation, TESObjectCELL* a_cell, bool a_unk4, bool a_unk5)
		{
			static REL::Relocation<decltype(&PlayerCharacter::MoveToCell)> func{ REL::ID(406911) };
			func(this, a_position, a_rotation, a_cell, a_unk4, a_unk5);
		}
		...
		BipedAnim*                           _1stPersonBipedAnim;             // 858
		Animation*                           _1stPersonAnimation;             // 860
		NiPointer<NiAVObject>                _1stPerson3D;                    // 868
		float                                eyeHeight;                       // 870
		...
		bool                                 beingChased;                     // A54
		BSSimpleList<MAP_MARKER_STRUCT*>     mapMarkerList;                   // A58
		TESWorldSpace*                       mapWorld;                        // A68
		bool                                 ufoMode;                         // A70
		float                                ufoCameraHeading;                // A74
		float                                ufoCameraPitch;                  // A78
		NiPoint3                             ufoCameraPos;                    // A7C
		std::uint32_t                        selectedSpellCastSoundID;        // A88
		TESGameSoundHandle*                  selectedSpellCastSound;          // A90
		TESGameSoundHandle*                  magicFailureSounds[6];           // A98
		BSSimpleList<TESObjectREFR*>         droppedRefList;                  // AC8
		NiTMap<std::uint32_t, std::uint8_t>  randomDoorSpaceMap;              // AD8
		NiPointer<NiPointLight>              _1stPersonLight;                 // AF8
		NiPointer<NiPointLight>              _3rdPersonLight;                 // B00
		std::int32_t                         numberActorsInCombat;            // B08
		float                                skillAdvancementUses[21];        // B0C
		bool                                 loadingFromMainMenu;             // B60
		bool                                 insufficientChargeMessageShown;  // B61
		float                                dropAngleMod;                    // B64
		float                                lastDropAngleMod;                // B68
		float                                playerDeathReloadTime;           // B6C
		bool                                 shouldRefreshClairvoyance;       // B70
		NiPointer<NiCamera>                  chaseCameraPtr;                  // B78
	};
